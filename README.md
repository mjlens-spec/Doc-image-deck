...
![version](https://img.shields.io/badge/version-2.0.2-blue.svg) ![license](https://img.shields.io/badge/license-MIT-green.svg) ![platform](https://img.shields.io/badge/platform-macOS%20%7C%20Windows-lightgrey.svg)
...
11. **风格统一、页面有变化**：每页单独生成，模型看不到其他页，所以变化逐页写进提示词：页面底面、强调方式、配图取景、辅助图形按变化库轮换，相邻两页不同；长稿里同一骨架至少隔两页，阅读稿每 6 页至少一页低密度或深色页；生成后用 `deck variety` 查版面相似度。
12. **字体可还原**：设计方向限定字体大类（黑体、宋体、楷体、圆体、行楷），正文只用黑体或宋体，不用花式字，可编辑版能对上思源黑体、思源宋体。
13. **生图质检**：OCR 核对缺字、预留角落、多出的文字和标题漂移；只错一两个字时局部改字、其余像素不变；最后交给新开的子 agent 对照视觉规划逐页审图，同一问题出现在 3 页以上时改方向或提示词，不逐页修。

## 安装

只想让 Codex 自动装好：把 [docs/install-codex-quick.md](docs/install-codex-quick.md) 发给 Codex，说「按这份文档安装」。
...
| `deck variety <项目>` | 同质化检查：页面之间的版面相似度 |
...
| 2.0.2 页面变化：同一份 15 页阅读稿、同一方向 | 版面相似度均值 1.2.1 为 0.491，逐页变化后两轮为 0.439、0.419；装饰上的变化可见，但每页都是高密度图示时整体观感仍接近，节奏页（低密度、深色）作用更大 |
