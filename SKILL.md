...
| 阶段 | 产出 | 停下来等用户 |
|---|---|---|
| 1 白板稿 | 故事线（标题串读、章节、原文来源）→ `outline.json` 正文与逐页视觉规划 → 用 humanizer-zh 去 AI 味 → 标点整理 → 白板稿 PPTX：终稿文字、结构、画法与版面、配图建议，不做设计 | **确认点 1**：确认故事线与白板稿，同时索取视觉参考、品牌材料和 Logo |
| 2 设计方向与生图 | 品牌 VI 与 Logo 调研 → 3 个完全不同的设计方向样张（封面构图也各不相同）→ 用户选定 → 全量逐页整页图片 → 文字核对与子 agent 审图 | **确认点 2**：选方向 |
...
`deck_mode` 是这套稿的用途：`read`（阅读稿，发给对方自己看，页面要脱离讲者也能看懂）或 `present`（演讲稿，现场讲，一页一个观点、细节进讲稿）。按用户的说法判断；没说明时用 `read`，用户说要上台讲时用 `present`，并可加 `"duration_min": 30`（演讲分钟数）。读法决定每页字量上限和节奏检查。用户限定页数或不要章节页时，加 `"max_pages": 15`（附录不计）、`"section_pages": false`，`deck visual` 会检查。`suffix` 用于所有交付文件名（`<name>_白板稿_<suffix>.pptx`、`_图文版_`、`_可编辑版_`）；在 Codex 中按用户在 Codex 的命名习惯（如 `OC_0925A`）。`source` 是原文档的相对路径，查找视觉参考时会一并搜索它所在的目录。`brand_materials` 列客户提供的品牌材料（也可以直接放进 `01_设计方向/品牌材料/`）；`logos` 在阶段 2 用 `deck logo` 生成，路径可以相对项目目录。`brands` 里的品牌会写进提示词的禁画清单，避免生图自己画 Logo。

## 阶段 1 · 白板稿

1. **故事线（先于正文）**：通读文档，先只写骨架：`outline.json` 顶层 `storyline`（`thesis` 全稿一句话结论、`mode` 论证方式、`dropped` 不上屏的原文章节及理由），每页只写 `kind`、`chapter`（所属章节）、`title`（结论式标题）、`visual.message`、`source`（来自原文哪一节、哪张表图；原文没有的写「补充：出处」）。运行 `$S/deck storyline <项目>`，改到通过，再写正文。它检查：章节连续、3–7 章；每个 `source` 都能在原文里找到；原文的一、二级标题都有对应的页或写进了 `dropped`；封底不是「谢谢」页；标题不写「本页」「接下来」这类话；阅读稿 8 页以上有执行摘要（`kind: summary`，`covers` 按章节顺序）；同一指标在各页数值一致。结果写在 `00_白板稿/故事线.md`（标题串读、原文来源、原文覆盖）。方法见 `references/01_白板稿.md`「故事线」。
2. **正文**：按 `references/01_白板稿.md` 的结构补齐每页的终稿正文、`layout_hint`（版面建议）、`image_hint`（配图建议）、`tone`（明暗）、讲稿。文字按宿主的写作规范成稿，达到可以直接对客户使用的程度；生图阶段逐字照搬这里的文字，之后很难再改。字量按构图骨架和读法控制（容量表见 `references/01_白板稿.md`「每页容量」），`deck visual` 会拦下超量的页；超出时拆成续页或换骨架，阅读稿的明细放进附录页（`kind: appendix`，排在封底之后，编号 A1、A2）。
3. **逐页视觉规划**：分拆定稿后，逐页分析这一页在说什么、文字之间是什么关系，写 `visual`：`message`（观众要看懂的一件事）、`structure`（信息结构：流程、闭环、漏斗、时间轴、对比、构成等）、`form`（具体画法：图形、方向、比例、哪段文字落在哪个图形上）、`focal`（唯一的视觉焦点）、`skeleton`（构图骨架）、`motif`（配图母题）。相邻两页骨架不同，同一骨架不超过内容页的四分之一，「并列要点」只用于真正平行的内容；演讲稿每 3 页至少一页低密度页（大字论断或大数字）。画法里不写图标，除非图标本身表达内容；要画时写明它代表什么。方法和两张可选表见 `references/01_白板稿.md`「逐页视觉规划」。这一步决定全稿是否雷同：只给文字和一句版面建议，模型会把多数页画成「标题 + 卡片」。
...
5. `$S/deck whiteboard <项目>`：故事线未通过、有文案没做去 AI 味时拒绝运行。先做**标点整理**（标题和短句去掉句号，只有详细描述和成段文字保留句号，规则见 `references/01_白板稿.md`），再生成白板稿 PPTX 和 `outline.md`，最后检查视觉规划并写 `00_白板稿/视觉规划.md`（也可单独运行 `$S/deck visual <项目>`）。标点改动记在 `00_白板稿/标点整理.md`；其中列出的「列表内句号不统一」要看一下，按需改写后重跑。视觉规划有 ✗ 项时改 `visual` 直到通过（`deck prompts` 会拒绝未通过的稿子）。打开 PPTX 检查有没有溢出、漏页。
...
7. **确认点 1（必须停下）**：在一条消息里给用户：
   - 故事线（放在第一项）：全稿结论、按章节分组的标题串读、原文里有意不上屏的内容（`dropped`）和原文以外的补充（`故事线.md`）。请用户先看论证顺序和取舍。
...
1. **品牌 VI 与 Logo 调研（定方向前必须）**：设计方向要建立在客户品牌上，先花 10–15 分钟做快速调研，方法见 `references/02_设计方向与生图.md`「品牌调研与 Logo」。
   ...
   - 填写 `品牌调研.md` 六节（资料来源、品牌色、Logo、字体与版式、视觉风格与禁忌、对三个方向的约束），第六节另起一行写「行业常见款：」，描述这个行业的提案最常见的样子（三个方向里最多一个接近它）；运行 `$S/deck brand <项目> check` 直到通过。
...
3. **写 3 个方向**：`01_设计方向/A|B|C/direction.json`。三个方向都要遵守 `品牌调研.md` 第六节的约束，每个方向用 `brand_fit` 写清它怎样用品牌色、Logo 和品牌风格；在此之上三个方向要**完全不同**：
   - `dims` 的字体、配色、版式语法、配图、质感、图示语言六项两两都不一样；`axes` 从固定选项里给字体、版式语法、质感、图示语言各选一个（两两不同），配色策略（克制、一色主导、多色、满铺）三个方向至少用两种；`imagery_mode`（写实摄影、产品静物、插画、纯图形、材质肌理、三维渲染、纯文字排版）三个方向各用一种。
   - `cover` 写这个方向自己的封面构图（`layout`）和封面主体（`motif`），三个方向两两不同。样张和全量生图的封面都用它，不用大纲里的封面版面建议，所以三张封面不会是同一个构图换配色。
   - `temperature`：`safe`（接近品牌调研里写的「行业常见款」）或 `distinct`（与它拉开），三个方向不能都是 `safe`。
   - `type_classes`：字体大类，只用常规、常见的字体。`body`（正文）只能是 `hei` 黑体或 `song` 宋体；`title`（标题）和可选的 `accent`（几个字的点缀）可以用 `hei`、`song`、`kai` 楷体、`yuan` 圆体、`xingkai` 行楷。提示词禁止描边、立体、变形、涂鸦、笔刷特效等花式字。可编辑版用思源黑体、思源宋体还原：黑体、宋体、圆体对得上；楷体、行楷只能近似或留在底图，需要整套都能改字时标题也用黑体或宋体。
   - `variety`（可选）：页面变化库，四项各 2–6 个英文短句：`surface` 页面底面、`accent` 强调方式、`framing` 配图取景、`device` 辅助图形，第一项是默认。不写就用通用选项。
   - `icons` 默认 `none`（不画图标）；只画视觉规划里点名的图标用 `planned`；插画方向可以用 `illustrative`。风格段落和 `imagery` 里不要写 pictograms、icons 之类的词，否则模型会给每个要点配一个线描图标。
   - 提示词默认禁画塑胶质感三维物体、玻璃拟态、发光线条、白底青蓝渐变、带投影的悬浮卡片、标题下的装饰短线和非序列内容的编号徽章；方向确实需要其中某项时写进 `allow`（`imagery_mode` 为 `3d` 时自动放开三维）。
   
   同时都要贴合内容和客户行业。按参考建立的方向写 `"source": "reference"` 和 `"refs"`，自行设计的写 `"source": "original"`。字段说明见 `references/02_设计方向与生图.md` 第 3 节。
4. `$S/deck directions <项目>` 检查品牌调研是否完成、三个方向是否都有 `brand_fit` 且完全不同（含封面构图、`axes`、配色策略），不通过就改到通过，同时生成 `01_设计方向/方向说明.md`（给用户看的对比表，含「品牌呼应」「封面」「与行业常见款」几行）。
5. **出样张**：每个方向选 2 页，封面 + 一页以图示为主的内容页（视觉规划里骨架为 `diagram`、信息最密的一页）：
   ...
   `$S/deck directions <项目> --samples`：对样张做文字核对，把结果写进 `方向说明.md` 的「样张核对」表。
6. **确认点 2（必须停下）**：把方向对比图、`方向说明.md`（含样张核对）和品牌调研的要点（品牌色、Logo 预览、行业常见款）给用户，请用户选定方向（也可以指定混合调整，如「A 的版式 + C 的配色」，按要求改写方向后重出样张）。三张封面构图各不相同；内容样张用的是同一份视觉规划，构图骨架相近是正常的，比较的是字体、配色、图示语言、配图和质感。结束本轮等用户回复。
7. **全量生图**：先按选定方向复核视觉规划：`motif` 改成适合该方向配图方式的题材，样张暴露的问题（如图示画得太满、焦点不明显）写回相关页的 `form`，运行 `$S/deck visual <项目>` 通过后再生成提示词。
   ...
   `deck prompts` 同时按方向的变化库给每页分配一组变化（页面底面、强调方式、配图取景、辅助图形），相邻两页四项都不同，六页内不重复同一组合，写进每页提示词和 `02_生图/prompts/变化规划.md`；某页要指定时在 `visual.variant` 里写。每页是单独生成的，模型看不到其他页，所以「多一点变化」这类笼统要求不起作用，变化要这样逐页写明。
   样张作为风格参考图附上，保证全稿风格一致；选定方向来自用户参考时，参考图也会附上（每次最多 3 张）。页数多时后台运行，每 10 页约 3–5 分钟。可选：`--skeleton-refs`（用了两页以上的构图骨架先各生成一页，其余同骨架的页以它为参考，沿用网格、页眉和字号，不照搬内容），`deck prompts … --corner-guide`（另附预留角落的定位图）；这两项的效果还在对比测试中，标题漂移或压角较多时再用。
8. **文字核对**：`$S/deck textcheck <项目> 02_生图/prompts 02_生图/raw`。
   - `MISS`（缺字、错字）和 `CORNER`（预留角落里有字）的页先放大看图：OCR 偶尔漏读大标题，确认文字在图上就不必重生成。
   - 只错一两处字（命令输出会列出）：先试 `$S/deck fix <项目> 02_生图/prompts 02_生图/raw --pages p05`，只改错字所在的区域，其余像素不变；改完自动重新核对，没有改善就恢复原版本，改用整页重生成。长句里只错一个字时核对结果是 `NEAR` 而不是 `MISS`：放大看图确认确实错了，再加 `--near --only <这句里的几个字>` 只改这一句。
   - 其他问题页用 `deck gen … --pages` 重新生成，最多两轮；两轮仍不过，改写该页版面建议后再生成。`CORNER` 多半是脚注落进了 Logo 角，在版面建议里把脚注放进结论横栏右端或标题区。需要删改文字时先告诉用户，确认后 `deck approve` 再生成。
   - `EXTRA`：图上有文案以外的文字（画法说明里的词被画成标签、自行算出的百分比、英文装饰词）；`DRIFT`：标题位置或字号与同明暗的其他页不一致（风格漂移）。这两类看图确认后重生成。`NEAR` 多为 OCR 误差（如把「滞」读成「滯」），看图确认。
9. **审图**：`$S/deck audit <项目> init` 生成 `02_生图/审图/审图.json` 和 `审图说明.md`，交给一个新开的子 agent（没参与写提示词）逐页对照视觉规划看图：骨架是否落地、最醒目的是不是规划的焦点、箭头与比例是否对应文字关系、有没有多出的标签和规划外的图标、风格是否漂移。填完后 `$S/deck audit <项目> check`：同一类问题出现在 3 页以上时是全稿问题，先改方向或提示词再重生成，不逐页修。没有子 agent 时自己按 `审图说明.md` 审。再生成全稿联系表 `$S/deck sheet 02_生图/联系表 --raw 02_生图/raw --cols 3` 看一遍整体节奏和配图题材是否重复，并运行 `$S/deck variety <项目>`：按缩略图的边缘分布算页面之间的版面相似度，写 `02_生图/同质化检查.md`；均值高于 0.45 或列出版面相近的页时，给这些页换变化项（`visual.variant`）或换骨架后重生成。有问题的页重生成，并在 `02_生图/raw/selected.json` 里指定采用的版本。
...
3. 白板稿里 `kpis` 块的大数字配小号前缀或单位（「约 250 条」「310 万元」）会按白板稿自动切成前缀、数字、单位三段，各按自己的字号还原，不需要手工切分。高于页高 18% 的超大数字（整行以数字为主）留在底图，因为去字会在周围留下光晕，交付检查里列出。自动切分没切开的（不在 `kpis` 块里的大小字混排），在 `pages.<pid>.split_points` 里写切分点，x 取两者之间的空白处。渐变色的艺术字标题颜色可能取错，放进排除区留在底图。
...
- 三个方向容易只在配色上有差别、配图都是同一类题材，封面都是同一个构图；`deck directions` 会拦下六项维度、`axes`、配图方式或封面构图重复的方向。
...
- 页数多了每页都差不多：骨架只有 10 种，长稿里同一骨架会反复出现，每页又附同一张样张。长稿（8 页以上内容页）要求同一骨架至少隔两页，配图母题不能相近，提示词按变化规划逐页写明底面、强调方式、取景和辅助图形；生成后用 `deck variety` 查。
- 花式字：生图模型容易给标题加描边、立体、渐变或笔刷效果，可编辑版对不上字体。方向的 `type_classes` 限定字体大类，提示词禁止花式字；正文只用黑体或宋体。
- 每个要点前面一个线描图标：模型的默认习惯，方向描述里出现 pictograms、icons 时更明显。默认 `icons: none`，风格段落里不写图标，视觉规划只在图标本身表达内容时点名。
- 先写正文再补故事线，容易漏掉原文章节或把一个判断摊成好几页；先跑通 `deck storyline` 再写正文。
