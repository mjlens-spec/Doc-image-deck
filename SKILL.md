...
9. **审图**：`$S/deck audit <项目> init` 生成 `02_生图/审图/审图.json` 和 `审图说明.md`，交给一个新开的子 agent（没参与写提示词）逐页对照视觉规划看图：骨架是否落地、最醒目的是不是规划的焦点、箭头与比例是否对应文字关系、有没有多出的标签和规划外的图标、风格是否漂移。填完后 `$S/deck audit <项目> check`：同一类问题出现在 3 页以上时是全稿问题，先改方向或提示词再重生成，不逐页修。没有子 agent 时自己按 `审图说明.md` 审。再生成全稿联系表 `$S/deck sheet 02_生图/联系表 --raw 02_生图/raw --cols 3` 看一遍整体节奏和配图题材是否重复。有问题的页重生成，并在 `02_生图/raw/selected.json` 里指定采用的版本。
