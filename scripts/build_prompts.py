...
prompt = slide spec + direction style block (light or dark) + this slide's composition, imagery and
verbatim text + fixed rules (only the listed text, no logo, no page number, reserved corners for the
logo and page number that compose.py adds later).
...
Refuses to run until the user has approved the whiteboard text (deck approve), see deckenv.whiteboard_approval.
"""
...
def build(cfg, outline, page, n, total, direction):
    ...
    lines.append('Imagery for this slide: ' + (page.get('image_hint') or 'no photograph; a clean typographic layout with at most small line icons'))
    lines.append('')
    lines.append('Text (verbatim) — render exactly these strings, in this reading order, with this hierarchy:')
    for role, s in E.page_strings(page):
        lines.append('- %s: "%s"' % (role, s.replace('"', '”')))
    lines.append('')
    lines.append('Text rules: render ONLY the text listed above, character by character. Simplified Chinese must be exact: no missing, '
