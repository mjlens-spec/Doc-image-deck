...
页数多时画面雷同、字体难以还原两个问题，以及一份交给 Codex 自动安装的精简说明。

- 页面变化：`deck prompts` 按设计方向的变化库给每页分配一组变化（页面底面、强调方式、配图取景、辅助图形），相邻两页不同、六页内不重复同一组合，写进提示词和 `02_生图/prompts/变化规划.md`；`direction.json` 可用 `variety` 换成自己的选项，页面可用 `visual.variant` 指定。每页单独生成、模型看不到其他页，所以变化要逐页写明
- 提示词：风格系统只固定底色、配色、字体、标题位置、边距和线宽，本页变化优先于参考图；参考图说明改为不照搬面板、色带、边框和装饰图形
- 长稿检查：8 页以上内容页时同一骨架至少隔两页；配图母题相近时提示；10 页以上的阅读稿每连续 6 页至少一页低密度页或深色页，否则提示
- 新增 `scripts/variety.py`（`deck variety`）：按缩略图边缘分布计算页面之间的版面相似度，列出过于相近的页，写 `02_生图/同质化检查.md`
- 字体大类：`direction.json` 新增必填的 `type_classes`（`hei` 黑体、`song` 宋体、`kai` 楷体、`yuan` 圆体、`xingkai` 行楷），正文只能是黑体或宋体；提示词只允许常规字形，禁止描边、立体、变形、笔刷特效等花式字。`deck editable` 按方向的字体大类限定拟合的字体族
- 新增 `docs/install-codex-quick.md`：交给 Codex 按步骤自动安装的精简说明
- 测试：同一份 15 页阅读稿，1.2.1 的版面相似度均值 0.491，2.0.2 两轮生成为 0.439、0.419；单元测试 66 项
- 升级：已有的 `direction.json` 要补 `type_classes`，否则 `deck directions` 不通过；运行环境没有变化
